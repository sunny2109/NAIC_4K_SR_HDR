...
lres_npz_dir = '/mnt/data2/lres_npz'
hres_npz_dir = '/mnt/data2/hres_npz'
predict_npz_dir = '/root/proj/NAIC/round1/dataset/test/lres_npz'
...
class VDM(nn.Module):
    # video denoise module via using 2D residual cnn
    def __init__(self, in_ch, feat_ch, out_ch):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(in_ch, feat_ch*2, kernel_size=1,
                      stride=1, padding=0),
            Mish(),
            nn.Conv2d(feat_ch*2, feat_ch*2, kernel_size=3,
                      stride=1, padding=1),
            Mish(),
            nn.Conv2d(feat_ch*2, feat_ch*2,
                      kernel_size=3, stride=1, padding=1),
        )

        self.layer2_1 = nn.Sequential(
            nn.Conv2d(feat_ch*2, feat_ch, kernel_size=3,
                      stride=1, padding=2, dilation=2),
            Mish(),
            nn.Conv2d(feat_ch, feat_ch, kernel_size=3,
                      stride=1, padding=2, dilation=2),
        )
        ...
        self.layer3 = nn.Sequential(
            nn.Conv2d(feat_ch*5, feat_ch*2, kernel_size=1,
                      stride=1, padding=0),
            Mish(),
            nn.Conv2d(feat_ch*2, out_ch, kernel_size=3,
                      stride=1, padding=1),
        )
    ...
    def forward(self, x):
        ...
        x2 = self.layer2_2(x)
        x3 = self.layer2_3(x)
        x = self.layer3(torch.cat((x, x1, x2, x3), dim=1))
        return x
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.vdm = VDM(in_ch=input_channel, feat_ch=32, out_ch=input_channel)

        self.conv = VConv2d(in_channels=input_channel, out_channels=32,
                              kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        vc_x = self.vdm(x)  # ch = input_channel

        vc_x = self.conv(vc_x)  # ch = 32
        ...
        fuse_lg = torch.cat((fuse_lg, vc_x), dim=1)  # ch = 32
        fuse_lg = self.decoder_conv(fuse_lg)  # ch = 64
        fuse_lg = self.decoder_rec(fuse_lg)  # ch = 64

        out = F.leaky_relu(self.pixel_shuffle(
            self.upconv1(fuse_lg)), 0.1)  # ch = 64
        out = F.leaky_relu(self.pixel_shuffle(
            self.upconv2(out)), 0.1)  # ch = 64
        ...
        return out[:, :, out_size[0]:out_size[1], out_size[2]:out_size[3]]
...
class Loss(nn.Module):
    ...
    def __init__(self, eps=1e-6, loss_lambda=5):
        super().__init__()
        self.eps = eps
