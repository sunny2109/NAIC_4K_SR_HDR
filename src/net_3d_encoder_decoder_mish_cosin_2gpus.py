...
class TVLossYUV(nn.Module):
    def __init__(self):
        super().__init__()
        print('Warning: TVLossYUV only used for yuv420p input and yuv422p output!')
    ...
    def forward(self, x):
        ...
        y = x[:,:4,:,:].view(batch_size,1,2,2,h_x,w_x).permute(0,1,4,2,5,3).reshape(batch_size,1,h_x*2,w_x*2)
        y_h_tv = torch.pow( (y[:,:,1:,:] - y[:,:,:-1,:])
            , 2).sum() / ((h_x*2-1) * w_x*2)
        y_w_tv = torch.pow( (y[:,:,:,1:] - y[:,:,:,:-1])
            , 2).sum() / (h_x*2 * (w_x*2-1))

        uv = x[:,4:,:,:].view(batch_size,2,2,h_x,w_x).permute(0,1,3,2,4).reshape(batch_size,2,h_x*2,w_x)
        uv_h_tv = torch.pow( (uv[:,:,1:,:] - uv[:,:,:-1,:])
            , 2).sum() / ((h_x*2-1) * w_x)
        uv_w_tv = torch.pow( (uv[:,:,:,1:] - uv[:,:,:,:-1])
            , 2).sum() / (h_x*2 * (w_x-1))

        loss = y_h_tv + y_w_tv + uv_h_tv + uv_w_tv
...
def criterion(output, target):
    return cbloss(output, target) + 1e-6 * tvloss(output)
# Loss Function end >>>
