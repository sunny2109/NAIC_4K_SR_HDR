...
class Net(nn.Module):
    def __init__(self):
        ...
        self.decoder_conv1 = VConv2d(
            in_channels=32, out_channels=input_channel, kernel_size=3, stride=1, padding=1)
        self.pac = PacConv2d(in_channels=input_channel, out_channels=32, kernel_size=3, stride=1, padding=1)
        self.decoder_conv2 = VConv2d(
            in_channels=32*3, out_channels=64, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        fuse_lg_pac = self.decoder_conv1(fuse_lg)  # ch = input
        fuse_lg_pac = self.pac(fuse_lg_pac, x) # ch = 32
        fuse_lg = torch.cat((fuse_lg_pac, fuse_lg, vc_x), dim=1)  # ch = 64
        fuse_lg = self.decoder_conv2(fuse_lg)  # ch = 64
